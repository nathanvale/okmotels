...
const ListItemListMock = ListItemList as jest.Mock

afterEach(() => {
  ListItemListMock.mockClear()
})
...
test('renders link to the discover page when there are no list items at all', () => {
  ...
  const discoverAnchor = noListItems.getByText(/discover/i, {
    selector: 'a',
  })
  expect(discoverAnchor).toHaveAttribute('href', '/app/discover')
})
...
test('renders link to the discover and finished pages when there are finished list items', () => {
  ...
  expect(finishedAnchor).toHaveAttribute('href', '/app/finished')
})

test('the given filter filters out finished books', () => {
  renderList()
  const {filterListItems} = ListItemListMock.mock.calls[0][0]
